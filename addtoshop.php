...
if ($total == $us['size']) {
    die(_("Sorry, but your shop is full!<br><a href=myshop.php>You will need to upgrade your shop's size."));
}
...
if ($_GET['price']) {
    ...
    if (100 == 500) {
        die(_("This item still belongs to your gang, you can't sell it."));
    } else {
        ...
        if ($quantity > $r['inv_qty']) {
            die(_("<center>You don't have this many of that item!</center>"));
        }

        if ($quantity < 1) {
            die(_("<center>A quantity of less than one? <br>Well that doesn't make much sense now does it?</center>"));
        }

        $newquant = $total + $quantity;
        if ($newquant > $us['size']) {
            die(_("Sorry, but you don't have enough room in your shop.<br><a href=myshop.php>click here to manage your shop</a>"));
        }
        ...
        print _("You have put this item in your shop.");
    ...
    if (mysql_num_rows($q) == 0) {
        print _("Invalid Item ID");
    } else {
        $r = mysql_fetch_array($q);

        if ($r['inv_qty'] > 1) {
            $quantask = _("Quantity:")." <input type='text' name='quantity' value='1'><br>";
            $quanttell = sprintf(_("You have <b>%d</b> of this item."),$r['inv_qty']);
        }
...
<div class='info_mid'><h2 style='padding-top:10px;'> "._("Putting item in your shop")."</h2></div>
...
"._("Price").": \$<input type='text' STYLE='color: black;  background-color: white;' name='price' value='0' /><br />
$quantask
