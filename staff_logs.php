...
<div class='generalinfo_simple'><br> <br><br>";
    ...
    for ($i = 1; $i <= $pages; $i++) {
        ...
        if ($s == $st) {
            ...
            print "<a href='staff_logs.php?action=atklogs&st=$s'>$i</a>&nbsp;";
        }
        if ($i % 25 == 0) {
            print "<br />";
        }
...
<table width=100% cellspacing=1 class='table'><tr style='background:gray'><th>Time</th><th>Who Attacked</th><th>Who Was Attacked</th><th>Who Won</th><th>What Happened</th></tr>";
    ...
    while ($r = $db->fetch_row($q)) {
        ...
        if ($r['result'] == "won") {
            print "<td>{$r['un_attacker']}</td><td>";
            if ($r['stole'] == -1) {
                print "{$r['un_attacker']} hospitalized {$r['un_attacked']}";
            } else if ($r['stole'] == -2) {
                print "{$r['un_attacker']} attacked {$r['un_attacked']} and left them";
            } else {
                print "{$r['un_attacker']} mugged \${$r['stole']} from {$r['un_attacked']}";
            }
            print "</td>";
        } else {
            print "<td>{$r['un_attacked']}</td><td>Nothing</td>";
        }
        print "</tr>";
    }
...
<div class='generalinfo_simple'><br> <br><br>";
    ...
    for ($i = 1; $i <= $pages; $i++) {
        ...
        if ($s == $st) {
            ...
            print "<a href='staff_logs.php?action=itmlogs&st=$s'>$i</a>&nbsp;";
        }
        if ($i % 25 == 0) {
            print "<br />";
        }
...
<table width='90%' cellspacing=1 class='table'><tr style='background:gray'><th>Time</th><th>Who Sent</th> <th>Who Received</th> <th>Sender's IP</th> <th>Receiver's IP</th> <th>Same IP?</th> <th>Item</th> </tr>";
    ...
    while ($r = $db->fetch_row($q)) {
        if ($r['ixFROMIP'] == $r['ixTOIP']) {
            $same = "<font color='red'>YES</font>";
        } else {
            $same = "<font color='green'>NO</font>";
        }
